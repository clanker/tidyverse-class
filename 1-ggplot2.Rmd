...
The examples I'll use is the Boston housing database
...
```{r cond-hist}
boston %>% 
  gather('key', 'value', -one_of('medv')) %>%
  mutate(price_gr = ntile(medv, 4)) %>%
  ggplot(aes(value, color = price_gr, group = price_gr)) +
...
boston_num %>% 
  cor() %>% 
  as_tibble() %>% 
  mutate(name = colnames(boston_num)) %>% 
  gather( , , -one_of("name")) %>% 
